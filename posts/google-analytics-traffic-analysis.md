...
This guide cuts through that. You will learn what each major metric actually measures, how the different traffic reports connect to each other, and how to use the data to make real decisions instead of just staring at graphs.
...
The number GA4 shows in most standard reports is **active users**, not total users. This matters: if you have 10,000 total users but only 6,000 active users, that 4,000-person gap represents people who landed and immediately left without engaging at all.
...
## Traffic Channels: Where Visitors Actually Come From

The Traffic Acquisition report in GA4 classifies your traffic into channels. These are not arbitrary buckets - each one tells you something specific about your marketing performance.
...
If you are running Google Ads but see very little Paid Search traffic in GA4, check three things: your auto-tagging settings in Google Ads, whether your UTM parameters are correctly formatted, and whether your GA4 property is linked to your Google Ads account.

### Direct

Direct traffic is widely misunderstood. It does not simply mean people typed your URL into their browser. It means GA4 could not identify where the visit came from. Common causes include:

- Email newsletters sent without UTM tracking
- PDF downloads with embedded links
- Mobile apps that strip referrer data
- HTTPS-to-HTTP referral stripping
- Dark social sharing (Slack, WhatsApp, Telegram, private messaging)

If your Direct traffic is unusually high - above 20-25% of total sessions for a site with active paid and email channels - you almost certainly have a tracking gap. The fix is applying UTM parameters consistently to every link you control.
...
For most B2B companies, organic social drives a small fraction of traffic but can drive high-quality leads when the content matches what your audience is searching for. Compare engagement rate and conversion rate from organic social against other channels before writing it off as a vanity channel.

### Paid Social

Clicks from paid social ads. Requires proper UTM tagging or native integrations (Facebook's Meta Pixel integration with GA4, for example) to attribute correctly.

### Referral

Traffic from other websites that link to you - blogs, directories, media coverage, partner sites. This is often undervalued. Referral traffic tends to arrive pre-qualified because the linking site's audience already has context about who you are.
...
The default primary dimension is **Session default channel group**, which is the broadest view. Change it to **Session source / medium** for a more specific breakdown - this shows exactly which platform plus medium combination is driving traffic (e.g., `google / organic`, `linkedin.com / referral`, `newsletter_march / email`).
...
Sort by **Engaged sessions** rather than Sessions. This filters out bot traffic and accidental one-second clicks that add no value to your analysis.

## Engagement Rate vs. Bounce Rate: What Changed in GA4

GA4 replaced the old bounce rate metric with engagement rate. They are the inverse of each other - an engagement rate of 65% means a bounce rate of 35%.
...
- Blog content: average engagement time above 2 minutes per session is healthy
- Landing pages: 45-90 seconds is typical for people who convert
- If your top organic landing pages have engagement time under 30 seconds, the content is not matching what searchers expected to find

## User Acquisition vs. Traffic Acquisition: Two Different Questions

These are two separate reports and they answer fundamentally different questions.

**Traffic Acquisition** tracks sessions - it credits whichever source drove a specific visit. If someone clicks a Google ad on Tuesday and then returns via direct on Thursday and converts, the conversion gets credited to the direct session (depending on attribution settings).
...
To check: go to Admin > Data display > Events. Any event with a star icon is marked as a key event. If you are a SaaS company and `generate_lead` or `sign_up` is not in that list, your conversion data is incomplete and every decision you make based on "conversions" will be wrong.

Once key events are set up, the analysis is straightforward:
...
- Report **users** when the question is about audience size: "How many people visited our site this month?"
- Report **sessions** when the question is about activity: "How much traffic did we generate?"
- Report **new users** when the question is about growth: "Are we reaching new people?"

Where this gets confusing: GA4's default "Users" metric in many reports is actually **active users**, which is lower than total users. If your number does not match what someone else exported from GA4, this discrepancy is usually the reason.

## The Direct Traffic Problem

Direct is the black hole of analytics. Industry data consistently shows that 15-30% of web traffic cannot be properly attributed, and a large chunk of that ends up in Direct.
...
- HTTP to HTTPS transitions without proper referrer headers
...
4. Check if engagement metrics changed on the affected pages - a drop in engagement time alongside a traffic drop can indicate a quality signal

When paid traffic drops, check:

1. Budget exhaustion (campaigns paused by budget)
2. Quality Score drops in Google Ads
3. Bid competition changes (cost-per-click up, same budget means fewer clicks)
4. Ad approval issues that paused delivery

## How to Find Your Traffic Sources in GA4

The navigation path is: Reports > Life cycle > Acquisition > Traffic acquisition.

You can view traffic sources classified by a different dimension by changing your primary dimension - click the arrow next to the current dimension name. Options include:

- **Session default channel group**: Broadest view, good for executive summaries
- **Session source / medium**: Most specific, shows exact platform and medium combinations
- **Session source**: Platform only (google, facebook, newsletter)
- **Session medium**: Type only (organic, cpc, referral, email)

To add a secondary dimension for deeper analysis, click the blue "+" sign next to the primary dimension. Adding "Session campaign" alongside "Session source / medium" shows which specific campaigns are driving traffic from each platform.

## Understanding Channel Quality Differences

Not all traffic channels convert equally - and the differences are usually dramatic once you look at session key event rates.

A typical B2B SaaS site might see:

- **Branded organic search**: Conversion rate 3-8% (people searching your company name are already interested)
- **Non-branded organic**: Conversion rate 0.5-2% (informational intent, needs nurturing)
- **Paid search**: Conversion rate 2-5% (intent signal in the query, but colder than branded)
- **Direct**: Conversion rate 1-4% (mix of returning visitors and tracking gaps)
- **Paid social**: Conversion rate 0.3-1.5% (interruption-based, lower intent)
- **Referral**: Conversion rate 1-4% (pre-qualified, but varies by source quality)

These are ranges, not guarantees. Your actual numbers will depend on your funnel, your offer, and how well your landing pages match the intent behind each channel. The value is in comparing your channels against each other, not against industry averages.
...
**Session key event rate 3x higher from one source vs. others**: Scale that source. The audience quality is clearly better there.
...
For teams that want a more automated version of this workflow, Toffu connects directly to Google Analytics and can run this analysis on a schedule, flagging anomalies and surfacing the specific numbers that need attention. [Start with a free account](https://toffu.ai/account/sign-up) to connect your GA4 property and run your first automated analysis.
...
**"Engagement rate means users are happy."** Engagement rate only measures minimum interaction thresholds. A user who spent 12 seconds on the page counts as engaged. Use average engagement time and conversion rate together with engagement rate for a real picture of content quality.

**"A low bounce rate is always good."** Not necessarily. A visitor who lands on a contact page, finds the phone number immediately, and leaves has a high bounce rate but a perfect outcome. Context matters.

## The Bottom Line on Traffic Analysis
